...
import { WheelStoreParameter } from './wheel.views.common';
...
const WheelView = observer(({ store }: WheelStoreParameter) => (
    <>
        ...
        <OwnerView viewModel={store.ownerVM} />
        ...
        <PlaceColorBetView parameter={store.colorVM} />
        <PlaceDozenBetView parameter={store.dozenVM} />
        <PlaceEighteenBetView parameter={store.eighteenVM} />
        <PlaceDoubleStreetBetView parameter={store.doubleStreetVM} />
        <PlaceSplitBetView parameter={store.splitVM} />
        <PlaceCornerBetView parameter={store.cornerVM} />
        <PlaceStraightUpBetView parameter={store.straightVM} />
        <PlaceTopLineBetView parameter={store.topLineVM} />
        {store.wheel ? (
            ...
            </div>) : (<p>Load the Wheel</p>)}
        </>
));
const DisconnectedView = observer(({ store }: WheelStoreParameter) => (
    <div>
...
const ContractNotLoadedView = observer(({ store }: WheelStoreParameter) => (
    <div>
...
const ConnectedView = observer(({ store }: WheelStoreParameter) => (
    <div>
        <h2>Connected to the blockchain!</h2>
        <p>Wallet Address: {store.signer ? store.signer.address : 'Error Loading Signer'}</p>
        <p>Contract Address: {store.wheelAddress ? store.wheelAddress : 'Wheel Not Loaded'}</p>
        {store.wheelAddress ? (
            <WheelView store={store} />
        ) : (
